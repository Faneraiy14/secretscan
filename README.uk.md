...
PHPUnit, 27 тестів: кожне правило окремо (включно з ентропійною),
редагування значення, заглушки, бінарні файли, рекурсія й виключення
папок, `secretscan:ignore`, дедуплікація regex/ентропія на тому самому
значенні (`tests/RuleDetectionTest.php`, `tests/ScanBehaviorTest.php`),
CLI-рівень (`--json`, exit-коди, `--help`) через реальний виклик
процесу (`tests/CliTest.php`).
